...
# USER CONFIGURATION
# ============================================================================
ROOT_DIR = "/ceph.groups/mshahbazi.grp/rsakata/HE_Nondeveloping3/LIF2TIF"
OUT_DIR = "/ceph.groups/mshahbazi.grp/rsakata/HE_Nondeveloping3/output/cellpose_boundary2.3"
...
VOXEL_SIZE_UM = (2.0, 0.3787879, 0.3786121) 
...
# Cellpose settings (2D segmentation, run independently in each Z slice).
...
PROFILE_LENGTH_FRACTION = 0.8   
PROFILE_HALF_WIDTH_UM = 1.0
PROFILE_STEP_UM = 0.5
...
# Normalization. "pair_dapi_mean" divides by the mean nuclear intensity of the
# two cells; "none" keeps raw values. Raw intensity is written either way.
#NORMALIZATION = "pair_dapi_mean"
NORMALIZATION = "none"
...
def find_interfaces(mask_z, dapi_z, pixel_size_um_yx):
    """
    ...
    cols = ["label_i", "label_j", "y1", "x1", "y2", "x2", "interface_y", "interface_x",
            ...
            "centroid_distance_um", "dist_to_i_um", "dist_to_j_um", "dapi_i", "dapi_j"]
    empty = pd.DataFrame(columns=cols)
    if mask_z.max() == 0:
        return empty, 0

    props = pd.DataFrame(
        regionprops_table(mask_z, intensity_image=dapi_z,
    ...
    df = pd.DataFrame({
        ...
        "gap_um": min_gap,
        "dapi_i": dapi[pair_lo], "dapi_j": dapi[pair_hi],
    })
...
def build_long_table(records):
    ...
    if NORMALIZATION == "pair_dapi_mean":
        factors = long["normalization_factor"].astype(float)
        factors = factors.where(np.isfinite(factors) & (factors > 0), 1.0)
...
def build_pair_table(long, interface_frames):
    ...
    if not meta.empty:
        meta_cols = keys + ["expanded_interface_length_um", "gap_um", "centroid_distance_um",
                            "n_interface_px", "dist_to_i_um", "dist_to_j_um",
                            "interface_y", "interface_x", "dapi_i", "dapi_j"]
...
def segment_stack(img, segmentation_indices, min_size_px):
    z_count = img.shape[0]
    masks = []
...
# PLOTTING
# ============================================================================
def plot_segmentation_preview(img, masks, name2idx, z_idx, outpath):
    dapi = percentile_norm(img[z_idx, name2idx[NUCLEAR_CHANNEL]])
    overlay = label2rgb(masks[z_idx], image=dapi, bg_label=0, bg_color=(0, 0, 0), alpha=0.45)
    y_size, x_size = dapi.shape
    fig, axes = plt.subplots(1, 2, figsize=image_figsize(y_size, x_size, ncols=2))
    ...
    axes[1].set_title(f"{int(masks[z_idx].max())} objects")
    for ax in axes:
        ax.set_axis_off()
    fig.tight_layout()
...
def process_single_file(img_path, sample_name):
    ...
    print(f"  > Z={z_count}, C={c_count}, Y={y_size}, X={x_size}; "
          f"pixel {y_um:.4f} um; Cellpose min_size={min_size_px} px "
          f"(= {MIN_OBJECT_AREA_UM2} um2)")

    segmentation_indices = [name2idx[n] for n in SEGMENTATION_CHANNELS]
    print(f"  > Segmenting {z_count} slices using {SEGMENTATION_CHANNELS}...")
    masks = segment_stack(img, segmentation_indices, min_size_px)
    print(f"  > {int(sum(m.max() for m in masks))} slice-local objects across "
    ...
    print("  > Finding interfaces and sampling ribbons...")
    ...
    for z in range(z_count):
        if masks[z].max() == 0:
            continue
        interfaces, n_candidates = find_interfaces(
            masks[z], img[z, name2idx[NUCLEAR_CHANNEL]], pixel_size_um_yx)
        ...
        norm = np.nanmean(np.stack([interfaces["dapi_i"].to_numpy(float),
                                    interfaces["dapi_j"].to_numpy(float)]), axis=0)
        norm = np.where(np.isfinite(norm) & (norm > 0), norm, 1.0)
    ...
    print(f"  > {total_pairs} of {n_candidate_pairs} candidate interfaces retained")
    if n_candidate_pairs and total_pairs < 0.5 * n_candidate_pairs:
        print(f"  [WARNING] {100*(1-total_pairs/n_candidate_pairs):.0f}% of candidate pairs were "
              f"filtered out. Check MAX_PAIR_GAP_UM ({MAX_PAIR_GAP_UM} um) and "
              f"MIN_INTERFACE_LENGTH_UM ({MIN_INTERFACE_LENGTH_UM} um) against the gap "
              f"distribution printed above.")

    z_preview = choose_preview_z(z_count, interface_counts)
    plot_segmentation_preview(
        img, masks, name2idx, z_preview,
