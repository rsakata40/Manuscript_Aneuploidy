...
# ===================================================
ROOT_DIR = '/ceph.groups/mshahbazi.grp/rsakata/HE_Nondeveloping5/LIF2TIF'   # Input folder
OUT_DIR = '/ceph.groups/mshahbazi.grp/rsakata/HE_Nondeveloping5/output/cellpose_boundary' # Output folder

os.makedirs(OUT_DIR, exist_ok=True)

XY_RES_UM = 0.5687380  # microns per pixel

# Channel Settings (0-based after reordering to ZCXY)
nucchannel = 0      # DAPI
optionalchannel = 4 # phalloidin
extractchannels = [optionalchannel, nucchannel] # Channels to pass to Cellpose

# Analysis Settings
masks_h5 = 0
slice2check = 10    # Slice index for preview plots
minsize2del = int(5.75 / (XY_RES_UM**2))
cellproblimit = 0 
flowlimit = 0.4
manualD = None
threshold = 3.0     # Threshold for GFP classification
line_width = 7      # Ribbon width for profile extraction
...
def process_single_file(img_path, sample_name):
    ...
    for z in range(Z):
        ...
        # Neighbor Finding
        neighbors = set()
        expanded_mask = maximum_filter(mask_z, size=7)
    ...
    channel_names =  ["DAPI", "GATA4", "NANOG", "GATA3", "ECAD"] # Ensure these match your image channels
    data_list = []
    ...
    for z in range(Z):
        ...
        for pair in pairs_in_z:
            ...
            dist = np.arange(len(rr)) * XY_RES_UM
    ...
    if len(cell_classes) > 0:
        ...
        for ax, cls in zip(axes, cell_classes):
            ...
            for ch, grp in sub.groupby("channel"):
                if ch in ["DAPI", "GATA4", "NANOG", "GATA3", "ECAD"]:
