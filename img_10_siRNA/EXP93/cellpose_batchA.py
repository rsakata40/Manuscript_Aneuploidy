...
# --------- USER CONFIG ---------
ROOT_DIR = '/ceph.groups/mshahbazi.grp/rsakata/EXP93/LIF2TIF/setA'       # folder with .tif/.tiff
OUT_DIR = '/ceph.groups/mshahbazi.grp/rsakata/EXP93/cellpose/output/setA/1_cellpose'
mask_dir = "/ceph.groups/mshahbazi.grp/rsakata/EXP93/cellpose/output/setA/1_cellpose_mask"
out_dir = OUT_DIR
...
CHANNEL_ORDER = ["dapi", "GATA3","NANOG", "ECAD"]  # edit names/order to match your data
NUC_NAME      = "dapi"                                # nuclear channel name used for segmentation

# Voxel size (µm) and optional min volume (µm³) filter for objects
voxel_size_um = (2.0, 0.7583174, 0.7583174)     # (z_um, y_um, x_um) — edit
...
# --------- Small helpers ---------
def load_zcyx(fpath, n_channels_expected):
    """Load a TIFF and return (Z,C,Y,X) using file metadata when available."""
    with tifffile.TiffFile(fpath) as tf:
        s = tf.series[0]
        arr = s.asarray()
        axes = s.axes  # e.g. 'ZYXC', 'ZCYX', 'TZCYX'

    # drop singleton axes we don't care about (T, S, ...)
    for ax in list(axes):
        if ax not in 'ZCYX':
            i = axes.index(ax)
            if arr.shape[i] != 1:
                raise ValueError(f"Unexpected non-singleton axis {ax} in {axes} {arr.shape}")
            arr = np.squeeze(arr, axis=i)
            axes = axes.replace(ax, '', 1)

    if sorted(axes) != ['C', 'X', 'Y', 'Z']:
        raise ValueError(f"Cannot interpret axes {axes} for {fpath}")

    order = [axes.index(a) for a in 'ZCYX']
    out = np.transpose(arr, order)

    Z, C, Y, X = out.shape
    if C != n_channels_expected:
        raise ValueError(f"{fpath}: got C={C}, expected {n_channels_expected}")
    if Y < 32 or X < 32:
        raise ValueError(f"{fpath}: suspicious Y,X = {Y},{X} — axes likely misread")
    print(f"{os.path.basename(fpath)}: {arr.shape} [{axes}] -> {out.shape} (Z,C,Y,X)")
    return out
...
def main():
    ...
    for fpath in files:
        ...
        img = load_zcyx(fpath, len(CHANNEL_ORDER))
        Z, C, Y, X = img.shape
        ...
        for p in props_all:
            ...
            cz_flip = (Z - 1) - cz          # top/bottom inverted
            row = dict(
                file_name=fname,
                sample=sample,
                ObjectID=lab,
                ...
                CentroidZflip_px=cz_flip,
                Z_slices=Z,                 # handy for sanity-checking across files
            )
            if vox_um3:
                ...
                row["CentroidZflip_um"] = cz_flip * z_um
