#!/bin/bash -l
#SBATCH --job-name=cyint58
#SBATCH --partition=gpu                 # gpu | agpu (RTX4090) | ml (A100)
#SBATCH --gres=gpu:8                   # set to 1–4 on agpu, 1–8 on gpu, 1–8 on ml (depending on cluster policy)
#SBATCH --cpus-per-task=8               # CPU workers for I/O & preprocessing
#SBATCH --time=48:00:00                 # Max allowed often 7-00:00:00
#SBATCH --mem=100G
#SBATCH --output=/ceph.groups/mshahbazi.grp/rsakata/EXP58/code/setA_T2/log/2000-%x-%j.out
#SBATCH --error=/ceph.groups/mshahbazi.grp/rsakata/EXP58/code/setA_T2/log/2000-%x-%j.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=rsakata@mrc-lmb.cam.ac.uk

# Optional: run as an array (one file per task). Uncomment and set range:
#SBATCH --array=0-44%4 #number of files-1 % GPU allowed

# --- Environment -------------------------------------------------------------
# Usage:
#   sbatch /ceph.groups/mshahbazi.grp/rsakata/EXP58/code/setA_T2/cytointensity.sh
...
cd /ceph.groups/mshahbazi.grp/rsakata/EXP58/code/setA_T2
...
/usr/bin/time -v \
python /ceph.groups/mshahbazi.grp/rsakata/EXP58/code/setA_T2/cytointensity.py
