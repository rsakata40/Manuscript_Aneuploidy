#!/bin/bash -l
#SBATCH --job-name=cyintT2
#SBATCH --partition=ml                  # gpu | agpu (RTX4090) | ml (A100)
#SBATCH --gres=gpu:1                   # keep 1
#SBATCH --cpus-per-task=8               # CPU workers for I/O & preprocessing
#SBATCH --time=48:00:00                 # Max allowed often 7-00:00:00
#SBATCH --mem=100G
#SBATCH --output=/ceph.groups/mshahbazi.grp/rsakata/EXP62/code/setB_T2/log/2000-%x-%j.out
#SBATCH --error=/ceph.groups/mshahbazi.grp/rsakata/EXP62/code/setB_T2/log/2000-%x-%j.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=rsakata@mrc-lmb.cam.ac.uk

# Optional: run as an array (one file per task). Uncomment and set range:
#SBATCH --array=0-68%4 #number of files-1 % GPU allowed

# --- Environment -------------------------------------------------------------
# Usage:
#   sbatch /ceph.groups/mshahbazi.grp/rsakata/EXP62/code/setB_T2/cytointensity.sh
...
/usr/bin/time -v \
python /ceph.groups/mshahbazi.grp/rsakata/EXP62/code/setB_T2/cytointensity.py
