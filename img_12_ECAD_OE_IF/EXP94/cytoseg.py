...
# ===================== CONFIG =====================
ROOT_DIR = '/ceph.groups/mshahbazi.grp/rsakata/EXP94/all/LIF2TIF'
OUT_DIR  = '/ceph.groups/mshahbazi.grp/rsakata/EXP94/all/cellpose/output/2_cytoseg'
...
MERGE_CHANNELS = [0, 1,3,4, 5]  # channels to merge into pre_combined
...
def filter_mask_per_slice(mask_bool_ZYX,
    ...
    strucT3d = _strucT3d(connectivity)
    ...
    for z in range(Z):
        ...
        # Label 2D components
        lab2d, nlab = ndi.label(m, structure=strucT3d)
        if nlab == 0:
            out[z] = m
            continue
